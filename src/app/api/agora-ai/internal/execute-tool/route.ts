...
/**
 * Endpoint interno usado por AgoraBackend (Cloud Run) para delegar la
 * ejecución de tools del agente al hub Next.js, donde viven Firestore,
 * MinIO, Forgejo y la conexión socket.io a los workers.
 *
 * Auth: el JWT del user (Bearer) + un X-Hub-Internal-Secret compartido
 * con el backend para evitar que clientes ajenos llamen este endpoint.
 */

const INTERNAL_SECRET = (process.env.HUB_INTERNAL_SECRET || '').trim();
...
export async function POST(request: NextRequest) {
  if (!INTERNAL_SECRET) {
    return NextResponse.json({ ok: false, error: 'HUB_INTERNAL_SECRET no configurado en el hub' }, { status: 500 });
  }
  const provided = request.headers.get('x-hub-internal-secret') || '';
